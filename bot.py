...
print("🔧 BOT FILE LOADED")  # 👈 проверка загрузки файла
...
async def start(message: types.Message):
    print("👤 USER PRESSED START")  # 👈 проверка входа
...
async def callbacks(callback: types.CallbackQuery):
    print(f"⚡ CALLBACK: {callback.data}")  # 👈 проверка кнопок
    ...
    if callback.data == "prod":
        ...
        await callback.message.edit_text("Опиши задачу 👇", reply_markup=nav_buttons())
...
async def handle(message: types.Message):
    print(f"💬 MESSAGE: {message.text}")  # 👈 проверка сообщений
    ...
    if "task" not in data:
        ...
        await message.answer("💰 Напиши бюджет:")
        return

    if "budget" not in data:
        data["budget"] = message.text
        await message.answer("⏳ Сроки?")
        return

    if "deadline" not in data:
        data["deadline"] = message.text
        await message.answer("📩 Оставь контакт:")
        return
...
# ▶️ запуск
async def main():
    print("🚀 BOT STARTED")  # 👈 ГЛАВНАЯ ПРОВЕРКА
